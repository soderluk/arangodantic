...
import arango.graph
import pydantic
from arango import GraphDeleteError
from arango.database import StandardDatabase
from pydantic import Field

from arangodantic._sync.models import DocumentModel, EdgeModel, Model
...
class Graph(ABC):
    ...
    @classmethod
    def get_graph(cls) -> arango.graph.Graph:
        return cls.get_db().graph(cls.get_graph_name())

    @classmethod
    def save(cls, model: Union[DocumentModel, EdgeModel], **kwargs):
        """
        ...
        if not model.rev_:
            ...
            model.before_save(new=True, **kwargs)
            ...
            try:
                ...
                if isinstance(model, EdgeModel):
                    response = graph.insert_edge(collection=collection_name, edge=data)
                else:
                    response = graph.insert_vertex(
            ...
            model.before_save(new=False, **kwargs)
            data = model.get_arangodb_data()
            try:
                if isinstance(model, EdgeModel):
                    response = graph.replace_edge(edge=data)
                else:
                    response = graph.replace_vertex(vertex=data)
    ...
    @classmethod
    def delete_vertex(cls, document: DocumentModel, ignore_missing=False) -> bool:
        """
        ...
        try:
            result: bool = cls.get_graph().delete_vertex(
    ...
    @classmethod
    def delete_edge(cls, edge: EdgeModel, ignore_missing=False) -> bool:
        """
        ...
        try:
            result: bool = cls.get_graph().delete_edge(
                data, ignore_missing=ignore_missing
            )
        except arango.exceptions.DocumentDeleteError as ex:
            if ex.error_code == ERROR_ARANGO_DOCUMENT_NOT_FOUND:
    ...
    def delete(
        ...
        """
        if isinstance(model, EdgeModel):
            return cls.delete_edge(edge=model, ignore_missing=ignore_missing)
        else:
            return cls.delete_vertex(document=model, ignore_missing=ignore_missing)

    @classmethod
    def ensure_graph(cls, **kwargs):
        """
    ...
    @classmethod
    def delete_graph(cls, ignore_missing: bool = False, drop_collections=None):
        """
