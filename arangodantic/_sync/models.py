...
class Model(pydantic.BaseModel, ABC):
    ...
    @classmethod
    def load(cls: Type[TModel], key: str) -> TModel:
        """
        ...
        """
        response = cls.get_collection().get(document={"_key": key})
        if response is None:
    ...
    def save(self, **kwargs) -> None:
        ...
        if not self.rev_:
            ...
            self.before_save(new=True, **kwargs)
            ...
            try:
                response = self.get_collection().insert(document=data)
            except arango.exceptions.DocumentInsertError as ex:
                if ex.error_code == ERROR_ARANGO_UNIQUE_CONSTRAINT_VIOLATED:
                    raise UniqueConstraintError(ex.error_message)
                raise
        else:
            # Update existing document
            self.before_save(new=False, **kwargs)
            data = self.get_arangodb_data()
            try:
                response = self.get_collection().replace(document=data)
            except arango.exceptions.DocumentReplaceError as ex:
                if ex.error_code == ERROR_ARANGO_UNIQUE_CONSTRAINT_VIOLATED:
                    raise UniqueConstraintError(ex.error_message)
                raise
    ...
    def delete(self, ignore_missing=False) -> bool:
        """
        Delete the document.
        ...
        data = self.get_arangodb_data()
        try:
            result: bool = self.get_collection().delete(
                document=data, silent=True, ignore_missing=ignore_missing
            )
        except arango.exceptions.DocumentDeleteError as ex:
    ...
    @classmethod
    def ensure_collection(cls, *args, **kwargs):
        """
        ...
        if not db.has_collection(name):
            db.create_collection(name, *args, **kwargs)

    @classmethod
    def delete_collection(cls, ignore_missing: bool = True):
        """
        ...
        **ignore_missing** is set to False.
        ...
        except arango.CollectionDeleteError as ex:
    ...
    @classmethod
    def truncate_collection(cls, ignore_missing: bool = True) -> bool:
        """
        ...
        **ignore_missing** is set to False.
        """
        try:
            cls.get_collection().truncate()
        except arango.CollectionTruncateError as ex:
            if ex.error_code == ERROR_ARANGO_DATA_SOURCE_NOT_FOUND:
                if ignore_missing:
                    return False
                else:
    ...
    def before_save(self, new: bool, **kwargs) -> None:
        """
    ...
    def find_one(
        ...
        *,
        ...
        results = cls.find(filters=filters, limit=limit, sort=sort).to_list()
        try:
...
class EdgeModel(Model, ABC):
    ...
    def ensure_collection(cls, *args, **kwargs):
        ...
        Ensure the collection exists and create it if needed.
        """
        return super(EdgeModel, cls).ensure_collection(edge=True, *args, **kwargs)
